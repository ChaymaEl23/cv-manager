...
const getNavItems = (role) => {
  if (role === 'hr') {
    return [
      ...
      { to: '/job-offers', label: 'Offres & candidats', icon: BriefcaseIcon },
    ]
  }
...
export default function ModernLayoutV2() {
      ...
      <div className="mx-auto flex min-h-screen max-w-[1800px] gap-6 px-4 py-4 lg:px-6">
        <aside className="sidebar-glow hidden w-[288px] shrink-0 flex-col rounded-[2rem] border border-white/8 p-6 text-white shadow-[0_28px_70px_rgba(13,22,46,0.36)] lg:flex">
          ...
          <div className="mt-10 rounded-[1.6rem] bg-white/6 p-4 ring-1 ring-white/8">
            ...
            <p className="mt-3 inline-flex rounded-full bg-white/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-slate-100">
              {user?.role === 'hr' ? 'HR' : 'Etudiant'}
            </p>
          </div>
          ...
          <div className="mt-6 space-y-4">
            <div className="rounded-[1.6rem] bg-[linear-gradient(135deg,rgba(76,123,255,0.26),rgba(82,176,255,0.16))] p-4 ring-1 ring-white/8">
              <p className="text-sm font-semibold">{user?.role === 'hr' ? 'Suivi recrutement' : 'Pret a postuler plus vite ?'}</p>
              <p className="mt-2 text-sm text-slate-200">
                {user?.role === 'hr'
                  ? 'Publiez une offre claire puis repondez aux candidatures avec un vrai suivi.'
                  : "Ajoutez votre experience et laissez l'IA preparer vos documents."}
              </p>
              <NavLink
                to={user?.role === 'hr' ? '/job-offers' : '/generate'}
                className="mt-4 inline-flex items-center gap-2 text-sm font-semibold text-white"
              >
                {user?.role === 'hr' ? 'Ouvrir le suivi' : "Ouvrir l'assistant"} <ArrowRightIcon className="h-4 w-4" />
              </NavLink>
            </div>
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-4">
          <header className="glass-panel rounded-[1.8rem] px-4 py-4 sm:px-6">
            <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
              <div>
                <p className="section-title mb-2">{user?.role === 'hr' ? 'Recrutement' : 'Tableau de bord'}</p>
                <h2 className="text-2xl font-extrabold tracking-[-0.04em] text-slate-800">
                  Bonjour, {user?.prenom || 'ami'}
                </h2>
                <p className="mt-1 text-sm text-slate-500">
                  {user?.role === 'hr'
                    ? 'Pilotez vos offres, vos candidats et vos places restantes dans le meme espace.'
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                <div className="flex items-center gap-3 rounded-[1.2rem] border border-slate-200/70 bg-white/75 px-4 py-3 shadow-[0_10px_24px_rgba(120,149,201,0.12)]">
                  ...
                  <span className="text-sm text-slate-400">{user?.role === 'hr' ? 'Candidatures...' : 'Rechercher...'}</span>
                </div>

                <NavLink to={user?.role === 'hr' ? '/job-offers' : '/generate'} className="primary-button whitespace-nowrap">
                  <PlusIcon className="h-5 w-5" />
                  {user?.role === 'hr' ? 'Nouvelle offre' : 'Generer CV'}
                </NavLink>
